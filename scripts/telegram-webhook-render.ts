...
async function main(): Promise<void> {
  ...
  if (!action || !["set", "info", "delete"].includes(action)) {
    throw new Error("Usage: tsx scripts/telegram-webhook-render.ts <set|info|delete>");
  }

  const token = requireEnv("TELEGRAM_BOT_TOKEN");

  if (action === "set") {
    const webhookUrl = requireEnv("RENDER_WEBHOOK_URL");
    ...
    console.log(`Render webhook set: ${webhookUrl}`);
    return;
  }
